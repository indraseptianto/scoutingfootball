...
export function normalizePlayer(item: SportmonksEntity) {
  ...
  const preferredFoot =
    metadataValue(metadata, ["preferred foot", "preferred_foot", "foot"]) ??
  ...
  return {
    ...
    contract_expires_at: findContractDate(item),
...
function metadataValue(metadata: SportmonksEntity[], names: string[]) {
  ...
  for (const item of metadata) {
    ...
    if (keys.some((key) => normalized.some((name) => key.includes(name)))) {
      return asString(item.value) ?? asString(item.data) ?? asString(item.description);
    }
  }
  return null;
}
...
function findContractDate(value: unknown): string | null {
  const date = findDateByKey(value, ["contract_expires_at", "contract_until", "contract_end", "contract_end_date", "end_date", "expires_at"]);
