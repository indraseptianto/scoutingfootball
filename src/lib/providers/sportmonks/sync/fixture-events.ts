...
import { normalizeFixtureEvent } from "../normalize/fixture-events";
import { finishSyncLog, startSyncLog, type SyncJobResult } from "./core";

const DEFAULT_FIXTURE_LIMIT = 10;

export async function syncFixtureEvents(fixtureId?: number): Promise<SyncJobResult> {
  if (fixtureId) {
    return syncEventsForFixture(fixtureId);
  }

  const supabase = getSupabaseServiceClient();
  const entity = "fixture-events";
  ...
  try {
    ...
    for (const fixture of fixtures ?? []) {
      try {
        const result = await syncEventsForFixture(Number(fixture.sportmonks_id));
...
async function syncEventsForFixture(fixtureId: number): Promise<SyncJobResult> {
  const supabase = getSupabaseServiceClient();
  const entity = `fixture-events:${fixtureId}`;
  const logId = await startSyncLog(entity);
  let recordsProcessed = 0;

  try {
    const response = await sportmonksFetch(sportmonksEndpoints.fixtureEvents(fixtureId));
    const items = Array.isArray(response.data) ? response.data : [];
    const rows = items.map(normalizeFixtureEvent).filter((r) => r.sportmonks_id);
    if (rows.length) {
      const { error } = await supabase.from("fixture_events").upsert(rows, { onConflict: "sportmonks_id" });
      if (error) throw error;
